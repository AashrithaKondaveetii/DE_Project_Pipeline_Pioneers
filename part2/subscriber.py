...
def upload_data_to_database(df):
    ...
    schema_name = "dataengineering"
    temp_table_name = f"{schema_name}.temp_trip"
    cur.execute(f"CREATE TEMPORARY TABLE {temp_table_name} (LIKE {schema_name}.trip);")
    cur.execute(
    ...
    output = io.StringIO()
    df[["trip_id", "route_id", "vehicle_id", "service_key", "direction"]].drop_duplicates().to_csv(output, header=False,
                                                                                                   index=False)
    output.seek(0)
    cur.copy_from(output, temp_table_name, sep=",", null="")
    cur.execute(
        f"INSERT INTO {schema_name}.trip SELECT * FROM {temp_table_name} ON CONFLICT DO NOTHING;"
    )

    output = io.StringIO()
    df[["tstamp", "latitude", "longitude", "speed", "trip_id"]].to_csv(output, header=False, index=False)
    output.seek(0)
    cur.copy_from(output, f"{schema_name}.breadcrumb", sep=",", null="")
